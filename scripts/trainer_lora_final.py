# scripts/bank_statement/trainer_lora_final.py (Version Ultime Simplifiée)
...
def train(args):
    ...
    # --- SIMPLIFICATION : On utilise 100% des données pour l'entraînement ---
    print(f"Chargement des données depuis {args.dataset_path}")
    with open(args.dataset_path, 'r') as f:
        full_dataset_data = json.load(f)
    
    # On met de côté 10% pour le test final, et on entraîne sur les 90% restants
    train_data = full_dataset_data[:int(len(full_dataset_data) * 0.9)]
...
if __name__ == '__main__':
    ...
    parser.add_argument("--num_train_epochs", type=int, default=10) # On fixe à 10 époques
